...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg">
      <div className="bg-surface border border-line rounded-3xl max-w-6xl w-full max-h-[92vh] overflow-y-auto p-6 sm:p-8 font-mono text-zinc-200 shadow-2xl relative">
        <div className="flex items-center justify-between border-b border-line pb-4 mb-6">
          ...
          <button
            onClick={onClose}
            className="px-3 py-1 bg-surface-2 border border-line text-white hover:bg-accent hover:text-black font-bold uppercase text-xs"
          >
            [CLOSE X]
          </button>
        </div>
              ...
              <form onSubmit={handleAdminLogin} className="space-y-4">
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full px-4 py-3 bg-black border border-line text-white text-xs font-bold text-center focus:outline-none focus:border-accent"
                  />
                </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap gap-2 border-b border-line pb-3">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-2 font-bold text-xs uppercase border rounded-full transition-all ${
                    activeTab === t.id
                      ? `${bgAccentClass} ${borderAccentClass}`
                      : 'border-line bg-black text-zinc-400 hover:text-white'
                  }`}
                >
                  {t.name}
                </button>
              ...
              <form onSubmit={handleSaveProfileSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" maxLength={60} value={profName} onChange={(e) => setProfName(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">LOCATION / DOMAIN TAG</label>
                    <input type="text" maxLength={80} value={profLoc} onChange={(e) => setProfLoc(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="email" maxLength={80} value={profEmail} onChange={(e) => setProfEmail(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">INSTAGRAM HANDLE</label>
                    <input type="text" maxLength={40} value={profInstagram} onChange={(e) => setProfInstagram(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2 items-center">
                    <input type="text" maxLength={500} value={profLogoUrl} onChange={(e) => setProfLogoUrl(e.target.value)} className="flex-1 px-3 py-2 bg-black border border-line text-white text-xs" placeholder="Image URL or upload below" />
                    <label className="px-3 py-2 bg-zinc-800 border border-line text-zinc-300 text-xs font-bold uppercase cursor-pointer hover:bg-zinc-700 flex items-center gap-1">
                    ...
                    {profLogoUrl && <img src={profLogoUrl} className="w-10 h-10 object-cover border border-line" alt="preview" />}
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">TAGLINE (MAIN ABOUT LINE)</label>
                  <textarea rows={2} maxLength={200} value={profTitle} onChange={(e) => setProfTitle(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">SHORT ABOUT PARAGRAPH</label>
                  <textarea rows={4} maxLength={1000} value={profBio} onChange={(e) => setProfBio(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs" />
                </div>

                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">ITALIC QUOTE / HEADLINE</label>
                  <textarea rows={3} maxLength={300} value={profHeadline} onChange={(e) => setProfHeadline(e.target.value)} className="w-full px-3 py-2 bg-black border border-line text-white text-xs italic" />
                </div>

                <div className="p-4 bg-surface-2 border border-line">
            ...
            {activeTab === 'typewriter' && (
              <div className="space-y-6">
                ...
                <div className="flex gap-2">
                  <input type="text" maxLength={60} placeholder="Add new role" value={newRoleInput} onChange={(e) => setNewRoleInput(e.target.value)} className="flex-1 px-3 py-2 bg-black border border-line text-white text-xs" />
                ...
                <div className="space-y-2">
                  {roles.map((role, idx) => (
                    <div key={idx} className="p-3 bg-black border border-line flex items-center justify-between text-xs font-bold">
            ...
            {activeTab === 'now_facts' && (
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {nowFocus.map((item, idx) => (
                      <div key={idx} className="p-4 bg-black border border-line space-y-2">
                        <input type="text" maxLength={40} value={item.title} onChange={(e) => { const updated = [...nowFocus]; updated[idx].title = e.target.value; setNowFocus(updated); }} className="w-full px-2 py-1 bg-canvas border border-line font-bold text-white text-xs uppercase" />
                        <textarea rows={2} maxLength={300} value={item.desc} onChange={(e) => { const updated = [...nowFocus]; updated[idx].desc = e.target.value; setNowFocus(updated); }} className="w-full px-2 py-1 bg-canvas border border-line text-zinc-300 text-xs" />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4 border-t border-line">
                  ...
                  <div className="flex gap-2">
                    <input type="text" maxLength={100} placeholder="Add new quick fact" value={newFactInput} onChange={(e) => setNewFactInput(e.target.value)} className="flex-1 px-3 py-2 bg-black border border-line text-white text-xs" />
            ...
            {activeTab === 'projects' && (
              <div className="space-y-6">
                ...
                {editingProj && (
                  <form onSubmit={handleProjectSubmit} className="p-4 bg-black border border-line space-y-3">
                    <h4 className="text-xs font-bold text-white uppercase border-b border-line pb-2">
                      {editingProj.id ? 'EDIT PROJECT' : 'CREATE NEW PROJECT'}
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input type="text" maxLength={80} placeholder="Project Title" required value={editingProj.title || ''} onChange={(e) => setEditingProj({ ...editingProj, title: e.target.value })} className="px-3 py-2 bg-canvas border border-line text-white text-xs" />
                      <input type="text" maxLength={40} placeholder="Category" value={editingProj.category || ''} onChange={(e) => setEditingProj({ ...editingProj, category: e.target.value })} className="px-3 py-2 bg-canvas border border-line text-white text-xs" />
                    </div>
                    <textarea placeholder="Project description" rows={3} maxLength={1000} value={editingProj.description || ''} onChange={(e) => setEditingProj({ ...editingProj, description: e.target.value })} className="w-full px-3 py-2 bg-canvas border border-line text-white text-xs" />

                    <div>
                      <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">PROJECT IMAGE</label>
                      <div className="flex gap-2 items-center">
                        <input type="text" maxLength={500} placeholder="Image URL or upload" value={editingProj.image_url || ''} onChange={(e) => setEditingProj({ ...editingProj, image_url: e.target.value })} className="flex-1 px-3 py-2 bg-canvas border border-line text-white text-xs" />
                        <label className="px-3 py-2 bg-zinc-800 border border-line text-zinc-300 text-xs font-bold uppercase cursor-pointer hover:bg-zinc-700 flex items-center gap-1">
                          <Upload className="w-3 h-3" />
                          {uploadingImage ? 'UPLOADING...' : 'UPLOAD'}
                        ...
                        {editingProj.image_url && <img src={editingProj.image_url} className="w-12 h-12 object-cover border border-line" alt="preview" />}
                      </div>
                    </div>

                    <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => setEditingProj(null)} className="px-3 py-1.5 border border-line text-zinc-300 text-xs font-bold uppercase">CANCEL</button>
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {projects.map((p) => (
                    <div key={p.id} className="p-3 bg-black border border-line flex items-center justify-between text-xs">
                      <div>
                        <span className="font-bold text-white uppercase">{p.title}</span>
                        <span className="text-zinc-500 ml-2">[{p.category}]</span>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setEditingProj(p)} className="px-2 py-1 bg-surface-2 text-zinc-300 hover:text-white">EDIT</button>
            ...
            {activeTab === 'skills' && (
              <div className="space-y-6">
                ...
                {editingSkill && (
                  <form onSubmit={handleSkillSubmit} className="p-4 bg-black border border-line space-y-3">
                    <h4 className="text-xs font-bold text-white uppercase border-b border-line pb-2">
                      {editingSkill.id ? 'EDIT TOOL' : 'ADD NEW TOOL'}
                    </h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input type="text" maxLength={50} placeholder="Tool Name" required value={editingSkill.name || ''} onChange={(e) => setEditingSkill({ ...editingSkill, name: e.target.value })} className="px-3 py-2 bg-canvas border border-line text-white text-xs" />
                      <input type="text" maxLength={40} placeholder="Category" value={editingSkill.category || ''} onChange={(e) => setEditingSkill({ ...editingSkill, category: e.target.value })} className="px-3 py-2 bg-canvas border border-line text-white text-xs" />
                    </div>
                    <div>
                      <label className="block text-[10px] text-zinc-400 uppercase mb-1">PROFICIENCY LEVEL: {editingSkill.level || 85}%</label>
                      <input type="range" min="10" max="100" value={editingSkill.level || 85} onChange={(e) => setEditingSkill({ ...editingSkill, level: Number(e.target.value) })} className="w-full accent-[#22d472]" />
                    </div>
                    <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => setEditingSkill(null)} className="px-3 py-1.5 border border-line text-zinc-300 text-xs font-bold uppercase">CANCEL</button>
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {skills.map((s) => (
                    <div key={s.id || s.name} className="p-3 bg-black border border-line flex items-center justify-between text-xs">
                      <div>
                        <span className="font-bold text-white uppercase">{s.name}</span>
                        <span className={`ml-2 font-bold ${textAccentClass}`}>{s.level}%</span>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setEditingSkill(s)} className="px-2 py-1 bg-surface-2 text-zinc-300 hover:text-white">EDIT</button>
            ...
            {activeTab === 'thoughts' && (
              <div className="space-y-6">
                ...
                {editingThought && (
                  <form onSubmit={handleThoughtSubmit} className="p-4 bg-black border border-line space-y-3">
                    <input type="text" maxLength={100} placeholder="Article Title" required value={editingThought.title || ''} onChange={(e) => setEditingThought({ ...editingThought, title: e.target.value })} className="w-full px-3 py-2 bg-canvas border border-line text-white text-xs font-bold" />
                    <textarea placeholder="Short summary" rows={2} maxLength={300} value={editingThought.summary || ''} onChange={(e) => setEditingThought({ ...editingThought, summary: e.target.value })} className="w-full px-3 py-2 bg-canvas border border-line text-white text-xs" />
                    <textarea placeholder="Full Article Content" rows={6} maxLength={5000} value={editingThought.content || ''} onChange={(e) => setEditingThought({ ...editingThought, content: e.target.value })} className="w-full px-3 py-2 bg-canvas border border-line text-white text-xs" />
                    <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => setEditingThought(null)} className="px-3 py-1.5 border border-line text-zinc-300 text-xs font-bold uppercase">CANCEL</button>
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {thoughts.map((t) => (
                    <div key={t.id} className="p-3 bg-black border border-line flex items-center justify-between text-xs">
                      <div className="max-w-md">
                        <span className="font-bold text-white uppercase">{t.title}</span>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => setEditingThought(t)} className="px-2 py-1 bg-surface-2 text-zinc-300 hover:text-white">EDIT</button>
            ...
            {activeTab === 'messages' && (
              <div className="space-y-4 max-h-[500px] overflow-y-auto">
                <div className="flex justify-between items-center">
                  ...
                  <input type="text" maxLength={50} placeholder="Search messages" value={messageSearch} onChange={(e) => setMessageSearch(e.target.value)} className="px-2 py-1 bg-black border border-line text-white text-xs" />
                </div>

                {messages.length === 0 ? (
                  <div className="p-6 text-center border border-line text-zinc-500 text-xs">
                  ...
                  messages
                    ...
                    .map((m) => (
                      <div key={m.id} className="p-4 bg-black border border-line space-y-2">
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto">
                      ...
                      <div key={g.id} className="p-3 bg-amber-950/30 border border-amber-800 space-y-2 text-xs">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <span className="ml-2 px-1.5 py-0.5 bg-surface-2 text-[9px] font-bold text-zinc-400">{g.badge}</span>
                          </div>
                        </div>
                ...
                {guestbook.filter(g => g.approved).length > 0 && (
                  <div className="space-y-2 pt-4 border-t border-line">
                    ...
                    {guestbook.filter(g => g.approved).map((g) => (
                      <div key={g.id} className="p-3 bg-black border border-line flex items-center justify-between text-xs">
                        <div>
                          <span className="font-bold text-white">{g.name}</span>
            ...
            {activeTab === 'backup' && (
              <div className="space-y-6">
                ...
                <div className="p-6 bg-black border border-line flex items-center justify-between">
                  <div>
