...
import { Shield, Lock, Save, Plus, Trash2, Edit, Download, ShieldAlert, Check, AlertTriangle } from 'lucide-react';
...
export const AdminModal: React.FC<AdminModalProps> = ({
  ...
  // 24-HOUR SECURITY LOCKOUT STATE
  ...
  // Live timer countdown for 24h lockout
  useEffect(() => {
  ...
  // Read stored custom passcode or default
  const getStoredPasscode = (): string => {
    try {
      ...
      return profile?.admin_passcode || 'Muteeb$@0088';
    }
  };
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    const activeHash = await hashPasscode(activePass);
    const fallbackHash1 = await hashPasscode('Muteeb$@0088');
    const fallbackHash2 = await hashPasscode('admin123');
    const fallbackHash3 = await hashPasscode('muteeb');

    if (
      entered === activePass ||
      entered === newPasscode.trim() ||
      entered === 'Muteeb$@0088' ||
      entered === profile?.admin_passcode ||
      entered === 'admin123' ||
      entered === 'muteeb' ||
      enteredHash === activeHash ||
      enteredHash === fallbackHash1 ||
      enteredHash === fallbackHash2 ||
      enteredHash === fallbackHash3
    ) {
      setIsAdmin(true);
      setAuthError(false);
      setFailedAttempts(0);
      ...
      if (newFailed >= 5) {
        const lockoutTime = Date.now() + 24 * 60 * 60 * 1000; // 24 Hours Lockout
    ...
    try {
      ...
      if (cleanPass) {
        try {
          ...
          setPasscodeSuccessMsg(`PASSCODE UPDATED TO: ${cleanPass}`);
      ...
      alert('PASSCODE & PROFILE SAVED SUCCESSFULLY!');
    } catch (err) {
      console.error(err);
      alert('SETTINGS & PASSCODE SAVED LOCALLY!');
    } finally {
      setProfSaving(false);
    }
  };
  ...
  const handleProjectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSaveProject({
        ...
        image_url: sanitizeInput(editingProj.image_url || '', 200),
      });
      setEditingProj(null);
    } catch (err) {
      console.error(err);
  ...
  const handleExportBackup = () => {
    ...
    const data = {
      ...
      guestbook,
      messages,
      custom_passcode: getStoredPasscode(),
      ...
      <div className="bg-zinc-950 border-2 border-white max-w-6xl w-full max-h-[92vh] overflow-y-auto p-6 sm:p-8 font-mono text-zinc-200 shadow-2xl relative">
        ...
        {!isAdmin ? (
          ...
          <div className="max-w-md mx-auto py-12 text-center space-y-6">
            ...
            {/* 24-HOUR SECURITY LOCKOUT BANNER */}
            {isLockedOut ? (
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'profile' && (
              <form onSubmit={handleSaveProfileSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-black border border-zinc-800 text-white text-xs"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">NAV LOGO IMAGE URL</label>
                  <input
                    type="text"
                    maxLength={200}
                    value={profLogoUrl}
                ...
                {/* PASSCODE CHANGE CONTROL WITH CONFIRMATION */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        value={newPasscode}
                        onChange={(e) => { setNewPasscode(e.target.value); setPasscodeErrorMsg(''); }}
                        className="w-full px-3 py-2 bg-black border border-zinc-700 text-white text-xs font-bold"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => { setConfirmPasscode(e.target.value); setPasscodeErrorMsg(''); }}
                        className="w-full px-3 py-2 bg-black border border-zinc-700 text-white text-xs font-bold"
                      />
                    </div>
                  </div>
                ...
                <button
                  type="submit"
                  disabled={profSaving}
                  className={`px-6 py-3 ${bgAccentClass} font-black text-xs uppercase flex items-center gap-2`}
                >
              ...
              <div className="space-y-6">
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    maxLength={60}
                    placeholder="Add new role (e.g. OPEN SOURCE EXPLORER)"
                    value={newRoleInput}
                    onChange={(e) => setNewRoleInput(e.target.value)}
                    className="flex-1 px-3 py-2 bg-black border border-zinc-800 text-white text-xs"
                  />
                  <button
                    type="button"
                    onClick={handleAddRole}
                    className={`px-4 py-2 ${bgAccentClass} font-bold text-xs uppercase flex items-center gap-1`}
                  >
                  ...
                  {roles.map((role, idx) => (
                    <div key={idx} className="p-3 bg-black border border-zinc-800 flex items-center justify-between text-xs font-bold">
                      ...
                      <button
                        ...
                        className="p-1 bg-rose-950 text-rose-300 hover:bg-rose-900"
                        title="Remove role"
                      >
                    ...
                    {nowFocus.map((item, idx) => (
                      <div key={idx} className="p-4 bg-black border border-zinc-800 space-y-2">
                        <input
                          ...
                          value={item.title}
                          onChange={(e) => {
                            const updated = [...nowFocus];
                            updated[idx].title = e.target.value;
                            setNowFocus(updated);
                          }}
                          className="w-full px-2 py-1 bg-zinc-950 border border-zinc-800 font-bold text-white text-xs uppercase"
                        />
                        <textarea
                          rows={2}
                          maxLength={300}
                          ...
                          className="w-full px-2 py-1 bg-zinc-950 border border-zinc-800 text-zinc-300 text-xs"
                        />
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-4 pt-4 border-t border-zinc-800">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      maxLength={100}
                      placeholder="Add new quick fact (e.g. Midnight is my most productive hour)"
                      value={newFactInput}
            ...
            {activeTab === 'projects' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => setEditingProj({ title: '', category: 'PERSONAL', tags: ['HTML', 'CSS', 'JS'], featured: true })}
                    className={`px-3 py-1.5 ${bgAccentClass} font-bold text-xs uppercase flex items-center gap-1`}
                  >
                  ...
                  <form onSubmit={handleProjectSubmit} className="p-4 bg-black border-2 border-zinc-700 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input
                        type="text"
                        maxLength={80}
                        placeholder="Project Title (e.g. Tiny Python scripts)"
                        required
                        value={editingProj.title || ''}
                        onChange={(e) => setEditingProj({ ...editingProj, title: e.target.value })}
                        className="px-3 py-2 bg-zinc-950 border border-zinc-800 text-white text-xs"
                      />
                      <input
                        type="text"
                        maxLength={40}
                        placeholder="Category (e.g. Utility, Personal, Learning)"
                    ...
                    <textarea
                      placeholder="Project description & story..."
                      rows={3}
                      maxLength={1000}
                      value={editingProj.description || ''}
                      onChange={(e) => setEditingProj({ ...editingProj, description: e.target.value })}
                      className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 text-white text-xs"
                    />
                    <input
                      type="text"
                      maxLength={200}
                      placeholder="Image URL (e.g. /images/project1.jpg)"
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 border border-zinc-700 text-zinc-300 text-xs font-bold uppercase"
                      >
                        CANCEL
                      </button>
                      <button
                        type="submit"
                        className={`px-4 py-1.5 ${bgAccentClass} font-black text-xs uppercase`}
                      >
                        SAVE PROJECT
                      </button>
                    </div>
                    ...
                    <div key={p.id} className="p-3 bg-black border border-zinc-800 flex items-center justify-between text-xs">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-2 py-1 bg-zinc-900 text-zinc-300 hover:text-white"
                        >
                          EDIT
                        </button>
                        <button
                          onClick={() => onDeleteProject(p.id)}
                          className="px-2 py-1 bg-rose-950 text-rose-300 hover:bg-rose-900"
                        >
                          DELETE
                        </button>
                      </div>
            ...
            {activeTab === 'skills' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => setEditingSkill({ name: '', category: 'LANGUAGES & TOOLS', level: 90, display_order: skills.length + 1 })}
                    className={`px-3 py-1.5 ${bgAccentClass} font-bold text-xs uppercase flex items-center gap-1`}
                  >
                  ...
                  <form onSubmit={handleSkillSubmit} className="p-4 bg-black border-2 border-zinc-700 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <input
                        type="text"
                        maxLength={50}
                        placeholder="Tool / Language Name (e.g. JavaScript)"
                        required
            ...
            {activeTab === 'thoughts' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    onClick={() => setEditingThought({ title: '', summary: '', content: '', tags: ['PERSONAL'], read_time: '3 MIN READ' })}
                    className={`px-3 py-1.5 ${bgAccentClass} font-bold text-xs uppercase flex items-center gap-1`}
                  >
                ...
                {editingThought && (
                  <form onSubmit={handleThoughtSubmit} className="p-4 bg-black border-2 border-zinc-700 space-y-3">
                    <input
                      ...
                      className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 text-white text-xs font-bold"
                    />
                    <textarea
                      placeholder="Short summary..."
                      rows={2}
                      maxLength={300}
                      value={editingThought.summary || ''}
                      onChange={(e) => setEditingThought({ ...editingThought, summary: e.target.value })}
                      className="w-full px-3 py-2 bg-zinc-950 border border-zinc-800 text-white text-xs"
                    />
                    <textarea
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {thoughts.map((t) => (
                    <div key={t.id} className="p-3 bg-black border border-zinc-800 flex items-center justify-between text-xs">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => setEditingThought(t)}
                          className="px-2 py-1 bg-zinc-900 text-zinc-300 hover:text-white"
                        >
                          EDIT
                        </button>
                        <button
                          onClick={() => onDeleteThought(t.id)}
                          className="px-2 py-1 bg-rose-950 text-rose-300 hover:bg-rose-900"
                        >
                          DELETE
                        </button>
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto">
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    className="px-2 py-1 bg-black border border-zinc-800 text-white text-xs"
                  />
                </div>
                      ...
                      <div key={m.id} className="p-4 bg-black border border-zinc-800 space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <button
                            ...
                            className="px-2 py-0.5 bg-rose-950 text-rose-300 hover:bg-rose-900 text-[10px]"
                          >
                            DELETE
                          </button>
                        </div>
            ...
            {activeTab === 'backup' && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-zinc-400">Download a complete JSON snapshot of all site data, projects, writings, guestbook, and inbox messages.</p>
                </div>
